...
def read_mf10(tape, mat, mt):
    ...
    >>> out = sandy.sections.mf10.read_mf10(tape, 4125, 16)
    >>> keys = {'MAT', 'MF', 'MT', 'ZA', 'AWR', 'LIS', 'LFS'}
    >>> assert keys.issubset(out)
    >>> assert {0, 1}.issubset(out["LFS"])
    >>> lfs0 = out["LFS"][0]
    >>> keys = {'QM', 'QI', 'IZAP', 'NBT', 'INT', 'E', 'XS'}
    >>> assert keys.issubset(lfs0)
    >>> expected = [8.92643e+06, 9.00000e+06, 9.50000e+06, 1.00000e+07, 1.05000e+07,
    ...
    ...        2.00000e+08]
    >>> assert lfs0["E"] == expected
    >>> expected = [0.       , 0.0194186, 0.162104 , 0.308902 , 0.435116 , 0.517474 ,
