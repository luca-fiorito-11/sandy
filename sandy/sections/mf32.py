...
def read_intg(df, NDIGIT, ipos):
        ...
        if NDIGIT == 2:
            ...
            II = int(series.C1[0:5])
            JJ = int(series.C1[5:10])
            KIJ1 = [series.C1[10],series.C2,series.L1,series.L2,series.N1,series.N2]
            KIJ = ''.join(str(e)for e in KIJ1)
                ...
                if LCOMP == 0:
                    ...
                    if ISR > 0:
                        ...
                        for k in range(NLS):
                            ...
                            keys = ["ER", "AJ", "GT", "GN", "GG", "GF", "DE²", "DN²", "DNDG", "DG²", "DNDF", "DF²", "DJDN", "DJDG", "DJDF", "DJ²"]
                            COVAR_PAR = [dict(zip(keys, items)) for items in sandy.utils.grouper(L.B, 18)]
                        ...
                        for k in range(NLS):
                            ...
                            keys = ["ER", "AJ", "GT", "GN", "GG", "GF", "DE²", "DN²", "DNDG", "DG²", "DNDF","DGDF", "DF²", "DJDN", "DJDG", "DJDF", "DJ²","0"]
                            COVAR_PAR = [dict(zip(keys, items)) for items in sandy.utils.grouper(L.B, 18)]
                            add.update({"COVAR_PAR":COVAR_PAR})
                            LCOMP0_NLS.update({k : add})
                            LCOMP0.update({"L":LCOMP0_NLS})
                    ...
                    if LRF == 1 or LRF == 2: 
                        ...
                        NRB = int(L.N2)
                        MPAR = int(L.L1)
                        keys = ["DET", "DWT", "GRT", "GIT", "DEF", "DWF","GRF", "GIF", "DEC", "DWC", "GRC", "GIC"]
                        RES_PAR = [dict(zip(keys, items)) for items in sandy.utils.grouper(L.B[:6*NRB], 12)]
                        add.update({"RES_PAR":RES_PAR})
                        tri = np.zeros((MPAR*NRB, MPAR*NRB))
                        tri[np.triu_indices(MPAR*NRB,0)] = np.array(L.B[6*NRB:])
                        COVAR_PAR = {
                        ...
                        if ISR > 0: 
                            ...
                            tri[np.triu_indices(NPARB*NPARB,0)] = np.array(L.B[6*NRB:])
                            COVAR_PAR = {
                                "COVAR_PAR": tri
                                }
                            add.update({"COVAR_PAR":COVAR_PAR})
                    ...
                    if LRF == 1 or LRF == 2: 
                        if ISR > 0:
                            ...
                            keys = ["ER", "AJ", "GT", "GN", "GG", "GF", "DER", "0", "0",  "DGN", "DGG", "DGF"]
                            RES_PAR = [dict(zip(keys, items)) for items in sandy.utils.grouper(L.B, 12)]
                            add.update({"RES_PAR":RES_PAR})
                            LCOMP2.update(add)
                            C, i = sandy.read_cont(df, i)
                            add = {
                        ...
                        if ISR == 1 : 
                            ...
                            keys = ["ER", "AJ", "GN", "GG", "GFA", "GFB", "DER", "0", "DGN",  "DGG", "DGFA", "DGFB"]
                            RES_PAR = [dict(zip(keys, items)) for items in sandy.utils.grouper(L.B, 12)]
                            add.update({"RES_PAR":RES_PAR})
                            C, i = sandy.read_cont(df, i)
                            add = {
