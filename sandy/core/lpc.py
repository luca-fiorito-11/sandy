    ...
    def from_endf6(cls, endf6):
        ...
        for mat, mf, mt in tape.data:
            ...
            for energy, vals in sec["LPC"]["E"].items():
                coefficients = [1] + vals["COEFF"] # add P0 coefficient, which is 1
...
class Tpd():
    ...
    def integrals(self):
        ...
        values = (self.data.iloc[:,:-1].values + self.data.iloc[:,1:].values) / 2.
        return pd.Series(np.dot(values, delta_cos), index=self.data.index, name="integral")
...
def lpc_to_tpd(coeff, cosines):
        ...
        """
        l = np.arange(coeff.size) # order of the polynomial
        a = (2*l + 1)/2 * coeff
