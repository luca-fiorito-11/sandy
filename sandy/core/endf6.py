...
#            for (mf,mt),text in sorted(tape.loc[mat].query('MT!=451'.format(mat)).TEXT.items()):
    ...
    def apply_perturbations(self, smps, 
                            processes=1,
                            temperature=0,
                            to_ace=False,
        ...
        **kwargs : keyword argument to pass to "tape.get_ace()".
        ...
        # kwargs are passed to the worker
        kwargs["filename"] = filename
        kwargs["to_file"] = to_file
        kwargs["verbose"] = verbose
        kwargs["temperature"] = temperature

        # njoy_kws are passed to get_pendf
        ...
        if to_ace:
            if processes == 1:
                ...
                for (nxs, pxs) in seq_xs:
                    pendf_ = pendf.perturb_xs(pxs)
                    outs[nxs] = to_ace_worker(
                        self.data,
                        pendf_.data,
                        nxs,
                        **kwargs,
            ...
            for (nxs, pxs) in seq_xs:
                pendf_ = pendf.perturb_xs(pxs)
                outs[nxs] = dict(endf6=self, pendf=pendf_)
                if to_file:
